...
criterion=LabelSmoothingCrossEntropy(epsilon=args.epsilon)
...
for epoch in tqdm(range(num_epochs), desc="Training epochs"):
    ...
    torch.save(res_dict, os.path.join(checkpoint_dir, 'model_{}.t7'.format(epoch+1)))
    if val_acc > best_val_acc:
        best_val_acc = val_acc
        torch.save(res_dict, os.path.join(checkpoint_dir, 'model_best.t7'))
...
checkpoint = torch.load(os.path.join(checkpoint_dir, 'model_best.t7'))
